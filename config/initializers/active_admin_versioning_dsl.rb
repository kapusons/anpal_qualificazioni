ActiveAdminVersioning::ActiveAdmin::DSL.module_eval do
  def versioning
    ...
    with_options only: :show do
      action_item :version, if: proc { application.versions.present? && !application.integration_required? && !application.draft? && (current_admin_user.super_admin? || current_admin_user.level_3?) } do
        link_to send("versions_admin_#{resource_instance_name}_path") do
          ::PaperTrail::Version.model_name.human
        end
      end
      action_item :version, only: :show, if: proc { application.versions.present? && !application.integration_required? && !application.draft? && (current_admin_user.super_admin? || current_admin_user.level_3?) } do
      ...
      sidebar ::PaperTrail::Version.model_name.human, if: proc { application.versions.present? && !application.integration_required? && !application.draft? && (current_admin_user.super_admin? || current_admin_user.level_3?) } do
        if versions.present?
